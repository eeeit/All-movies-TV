...
function toApiPlayRecord(record: PlayRecord): SharedPlayRecord {
  return {
    ...record,
...
function toApiFavorite(favorite: Favorite): SharedFavorite {
  return {
    ...favorite,
...
async function fetchPlayRecordsFromApi(): Promise<Record<string, PlayRecord>> {
  return apiClient.getPlayRecords();
}

async function fetchFavoritesFromApi(): Promise<Record<string, Favorite>> {
  return apiClient.getFavorites();
}

async function fetchSearchHistoryFromApi(): Promise<string[]> {
...
async function handleDatabaseOperationFailure(
  ...
  try {
    ...
    switch (dataType) {
      case 'playRecords':
        freshData = await fetchPlayRecordsFromApi();
        cacheManager.cachePlayRecords(freshData);
        eventName = 'playRecordsUpdated';
        break;
      case 'favorites':
        freshData = await fetchFavoritesFromApi();
        cacheManager.cacheFavorites(freshData);
        eventName = 'favoritesUpdated';
        break;
      case 'searchHistory':
        freshData = await fetchSearchHistoryFromApi();
...
export async function getAllPlayRecords(): Promise<Record<string, PlayRecord>> {
  ...
  if (STORAGE_TYPE !== 'localstorage') {
    ...
    if (cachedData) {
      ...
      // 缓存为空，直接从 API 获取并缓存
      try {
        const freshData = await fetchPlayRecordsFromApi();
        cacheManager.cachePlayRecords(freshData);
        return freshData;
      } catch (err) {
...
export async function savePlayRecord(
  ...
  if (STORAGE_TYPE !== 'localstorage') {
    ...
    // 异步同步到数据库
    try {
      await apiClient.savePlayRecord({
        key,
        record: toApiPlayRecord(record),
      });
    } catch (err) {
...
export async function deletePlayRecord(
  ...
  if (STORAGE_TYPE !== 'localstorage') {
    ...
    // 异步同步到数据库
    try {
      await apiClient.deletePlayRecord({ key });
    } catch (err) {
      await handleDatabaseOperationFailure('playRecords', err);
      throw err;
    }
...
export async function getSearchHistory(): Promise<string[]> {
  ...
  if (STORAGE_TYPE !== 'localstorage') {
    ...
    if (cachedData) {
      ...
      // 缓存为空，直接从 API 获取并缓存
      try {
        const freshData = await fetchSearchHistoryFromApi();
        cacheManager.cacheSearchHistory(freshData);
        return freshData;
      } catch (err) {
...
export async function addSearchHistory(keyword: string): Promise<void> {
  ...
  if (STORAGE_TYPE !== 'localstorage') {
    ...
    // 异步同步到数据库
    try {
      await apiClient.addSearchHistory({ keyword: trimmed });
    } catch (err) {
...
export async function deleteSearchHistory(keyword: string): Promise<void> {
  ...
  if (STORAGE_TYPE !== 'localstorage') {
    ...
    // 异步同步到数据库
    try {
      await apiClient.deleteSearchHistory({ keyword: trimmed });
    } catch (err) {
      await handleDatabaseOperationFailure('searchHistory', err);
    }
    return;
...
export async function getAllFavorites(): Promise<Record<string, Favorite>> {
  ...
  if (STORAGE_TYPE !== 'localstorage') {
    ...
    if (cachedData) {
      ...
      // 缓存为空，直接从 API 获取并缓存
      try {
        const freshData = await fetchFavoritesFromApi();
        cacheManager.cacheFavorites(freshData);
        return freshData;
      } catch (err) {
  ...
  if (STORAGE_TYPE !== 'localstorage') {
    ...
    try {
      await apiClient.saveFavorite({
        key,
        favorite: toApiFavorite(favorite),
      });
    } catch (err) {
...
export async function deleteFavorite(
  ...
  if (STORAGE_TYPE !== 'localstorage') {
    ...
    // 异步同步到数据库
    try {
      await apiClient.deleteFavorite({ key });
    } catch (err) {
      await handleDatabaseOperationFailure('favorites', err);
      throw err;
    }
...
export async function clearAllPlayRecords(): Promise<void> {
  ...
  if (STORAGE_TYPE !== 'localstorage') {
    ...
    // 异步同步到数据库
    try {
      await apiClient.deletePlayRecord();
    } catch (err) {
      await handleDatabaseOperationFailure('playRecords', err);
      throw err;
    }

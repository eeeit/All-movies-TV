...
import { withApiBase } from './api-url';

function getImageProxyRoutePrefixes(): string[] {
  return [
    `${apiRoutes.imageProxy}?url=`,
    `${withApiBase(apiRoutes.imageProxy)}?url=`,
  ];
}
...
export function processImageUrl(originalUrl: string): string {
  ...
  if (
    getImageProxyRoutePrefixes().some((prefix) => originalUrl.includes(prefix))
  ) {
    return originalUrl;
  }
  ...
  if (shouldAutoProxyImageUrl(originalUrl)) {
    return `${withApiBase(apiRoutes.imageProxy)}?url=${encoded}`;
  }
